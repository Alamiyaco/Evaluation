...
const S = {
  ...
  counts: {}           // pageKey → { late, todayN, upcoming, total }
};
let autoT;

// ── NAV ICONS ────────────────────────────────────────
const PAGE_ICONS = {
  evaluations:    "📋", generalEvents: "📅",
  employeeEvents: "👤", birthdays:     "🎂"
};
const PAGE_EMOJIS = {
  generalEvents: "📅", employeeEvents: "🎉", birthdays: "🎂"
};
...
/* ════════════════════════════════════════════════════
   MAPPERS
   ════════════════════════════════════════════════════ */
function findColIndex(headers, aliases, fallbackIndex){
  const idx = headers.findIndex(h => aliases.some(a => h === norm(a) || h.includes(norm(a)) || norm(a).includes(h)));
...
function mapEvals(rows){
  ...
  const idx = {
    name: findColIndex(headers,["اسم الموظف","الاسم","name","الموظف"],0),
    department: findColIndex(headers,["القسم","department","الادارة","الوحدة"],1),
    firstDate: findColIndex(headers,["التقييم الاول","التقييم الأول","first evaluation","تاريخ التقييم الاول","تاريخ التقييم الأول"],3),
    firstResult: findColIndex(headers,["نتيجة التقييم الاول","نتيجة الاول","نتيجة 1","النتيجة 1","result 1","نتيجة الأول","نتيجة التقييم الأول"],4),
    secondDate: findColIndex(headers,["التقييم الثاني","second evaluation","تاريخ التقييم الثاني"],5),
    secondResult: findColIndex(headers,["نتيجة التقييم الثاني","نتيجة الثاني","نتيجة 2","النتيجة 2","result 2"],6),
    thirdDate: findColIndex(headers,["التقييم الثالث","third evaluation","تاريخ التقييم الثالث"],7),
    thirdResult: findColIndex(headers,["نتيجة التقييم الثالث","نتيجة الثالث","نتيجة 3","النتيجة 3","result 3"],8)
  };

  return rows.slice(1)
    .filter(row => row.some(hasV))
    .map(row => ({
      name: (row[idx.name] || "").toString().trim(),
      department: (row[idx.department] || "").toString().trim() || "بدون قسم",
      evaluations:[
        {type:"التقييم الأول",key:"first",  date:toIso(row[idx.firstDate]),  result:row[idx.firstResult]},
        {type:"التقييم الثاني",key:"second", date:toIso(row[idx.secondDate]), result:row[idx.secondResult]},
        {type:"التقييم الثالث",key:"third",  date:toIso(row[idx.thirdDate]),  result:row[idx.thirdResult]}
      ]
    }))
